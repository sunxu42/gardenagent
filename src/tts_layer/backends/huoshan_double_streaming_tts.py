...
class HuoshanTTS(BaseTTS):
    ...
    async def _forward_loop(self):
        ...
        while self.is_processing and self._ws:
            try:
                ...
                if event == self._EVENT_SessionCanceled:
                    ...
                    if payload:
                        try:
                            ...
                            logger.info(f"句子语音生成开始: {self.current_text}")
                        except Exception as e:
                ...
                elif event == self._EVENT_TTSSentenceEnd:
                    logger.info(f"句子语音生成结束: {self.current_text}")
    ...
    async def finish_session(self, session_id: str ):
        ...
        try:
            ...
            payload = b"{}"
            await self._send_event(header, optional, payload)
            logger.info(f"会话结束请求已发送: {session_id}")
    ...
    async def cancel_session(self, session_id: str = None):
        ...
        try:
            ...
            payload = b"{}"
            await self._send_event(header, optional, payload)
            logger.info(f"会话取消请求已发送: {session_id}")
        except Exception as e:
