...
class AgentService:
    ...
    async def process_final_text(self, text: str):
        ...
        async for response in self.agent.achat(text):
            if response:
                if count == 0:
                    await self.publish_response("SENTENCE_START")
                await self.publish_response(response, index=count)
                count += 1
        await self.publish_response("SENTENCE_END")
        await self.end_session()

    
    async def publish_response(self, response: str, index: int=-1):
        message = {
            'text': response,
            'timestamp': time.time(),
            'index': index,
