...
from yard.configs.secrets import Secrets, load_secrets

from src.settings import UnifiedConfig, load_settings


def needs_media_secrets(settings: UnifiedConfig) -> bool:
    """输入或输出含 audio 时需要 ASR/TTS 凭证。"""
    modalities = set(settings.input_modality) | set(settings.output_modality)
    return "audio" in modalities


def apply_media_secrets(settings: UnifiedConfig, secrets: Secrets) -> UnifiedConfig:
