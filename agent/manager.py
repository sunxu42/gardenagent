...
from agent.memory.bootstrap import (
...
def _apply_subsystem(agent_manager, emotion, memory) -> None:
    """将 emotion / memory 子系统装配结果挂到 AgentManager 实例。"""
    agent_manager.emotion_service = emotion.service
    agent_manager.emotion_appraisal_middleware = emotion.appraisal_middleware
    agent_manager.tts_voice_type = emotion.tts_voice_type
    agent_manager.mem0_service = memory.mem0_service
...
class AgentManager:
    ...
    def __init__(self, config=None):
        ...
        self.config = resolve_agent_runtime(settings, secrets)
        self.langfuse_client, self.langfuse_handler = init_langfuse()

    @classmethod
    async def create(cls, config=None):
        agent_manager = cls(config)
        init_workspace(agent_manager.config.workspace_dir)

        agent_manager.tools = await load_mcp_tools(agent_manager.config)
        agent_manager.local_scheduler = LocalSchedulerService(agent=agent_manager)
        ...
        emotion = setup_emotion_subsystem(agent_manager.config)
        ...
        agent_manager.agent = create_deep_agent(
            model=create_glm_model(agent_manager.config),
            tools=agent_manager.tools,
            system_prompt=None,
            skills=[agent_manager.config.skills_dir],
            backend=backend,
            checkpointer=agent_manager._checkpointer,
            middleware=middleware,
        )

        from agent.memory.admin.user_data import register_agent_manager

        register_agent_manager(agent_manager)

        agent_manager.agent_input_queue = asyncio.Queue(maxsize=1000)
        agent_manager.agent_output_queue = asyncio.Queue(maxsize=1000)
        agent_manager._agent_input_processing = False

        agent_manager._worker_stop = asyncio.Event()
        agent_manager._worker_task = asyncio.create_task(
            agent_manager._agent_input_worker(),
            name="input_worker",
        )
        agent_manager._heartbeat_stop = asyncio.Event()
        agent_manager._heartbeat_task = asyncio.create_task(
            run_heartbeat_enqueue_loop(
                agent=agent_manager,
                stop_event=agent_manager._heartbeat_stop,
            ),
            name="heartbeat_enqueue",
        )
        start_memory_background_tasks(agent_manager)
