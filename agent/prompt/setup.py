"""Assemble the system-prompt middleware and its dependencies."""
...
from agent.middlewares.system_prompt import SystemPromptMiddleware
...
from agent.prompt.compose.module import load_manifest
from agent.prompt.renderers import RendererDeps


def setup_system_prompt_middleware(
    config: Any,
    *,
    emotion_service: EmotionService | None,
) -> SystemPromptMiddleware | None:
    """Build the sole system-prompt writer when enabled (default on)."""
    enabled = bool(getattr(config, "prompt_composer_enabled", True))
    if not enabled:
        return None
...
setup_prompt_composer = setup_system_prompt_middleware
