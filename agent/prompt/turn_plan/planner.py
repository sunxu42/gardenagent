"""Per-turn reply planner for Danhuang (strategy → ReplyPlan)."""
...
class ReplyPlan:
    ...
    # none | offer_choice_2plus | explain_only | structured_table
...
class DanhuangReplyPlanner:
    ...
    def build(self, ctx: "PromptContext") -> ReplyPlan:
        ...
        return ReplyPlan(
            ...
            presentation_hint=self._presentation_hint_for(ctx),
        )
    ...
    def _presentation_hint_for(self, ctx: "PromptContext") -> str:
        text = (ctx.user_text or "").strip()
        if any(keyword in text for keyword in ("为什么", "是什么", "什么意思", "怎么形成")):
            return "explain_only"
        if any(
            keyword in text
            for keyword in (
        ...
        if any(keyword in text for keyword in ("玩", "猜", "选", "难度", "继续", "谜题", "游戏")):
            return "offer_choice_2plus"
        if ctx.turn_type in {"play", "puzzle"}:
            return "offer_choice_2plus"
        strategy_tags = ctx.strategy_tags
        if strategy_tags is not None and getattr(strategy_tags, "mode", None) in {"play", "explore"}:
            return "offer_choice_2plus"
        return "none"
