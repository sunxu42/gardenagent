...
from yard.prompt.formatting import format_slice_value
from yard.prompt.modules import TIER_ORDER, PromptModule
from yard.prompt.renderers import RendererDeps, get_renderer, validate_manifest_renderers
from yard.prompt.slice import interpolate_template, load_yaml_source, resolve_slice
    ...
    def resolve(self, ctx: PromptContext) -> list[PromptModule]:
        ...
        for m in self._modules:
            if m.always or evaluate_when(m.when, ctx):
                out.append(m)
        out.sort(key=lambda mod: (TIER_ORDER.index(mod.tier) if mod.tier in TIER_ORDER else 99, mod.priority))
        return out
