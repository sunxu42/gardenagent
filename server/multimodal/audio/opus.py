...
class OpusCodecUtils:
    ...
    def opus_to_pcm(self, opus_data: bytes) -> bytes:
        ...
        if not opus_data:
            return b""
        try:
            return self.decoder.decode(opus_data, frame_size=self.frame_size)
        except Exception as e:
            logging.error(f"Opus解码失败: {e}")
            traceback.print_exc()
            return b""

    def pcm_to_opus(self, pcm_data: bytes) -> bytes:
        """编码单帧 PCM"""
        if not pcm_data:
            return b""
        try:
    ...
    # def pcm_to_opus_stream(self, pcm_data: bytes, end_of_stream: bool):
    #     """
    #     将连续的 PCM 流编码为一组 Opus 包。

    #     与 websocket 层的使用方式兼容：一次传入一段 PCM，
    #     按内部帧切分并返回一个 Opus 包列表。
    #     """
        # packets = []

        # def _collect(packet: bytes):
        #     if packet:
        #         packets.append(packet)

        # self.encode_pcm_to_opus_stream(
        #     pcm_data=pcm_data,
