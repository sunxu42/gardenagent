...
function updateScriptStatus(message, type) {
    ...
    if (statusElement) {
        ...
        statusElement.className = `script-status-overlay ${type}`;
...
// 耗时计算相关变量
let timingStartTime = null; // 手动开始计时的时间
let firstAudioPlayTime = null; // 第一个音频包开始播放时间
let firstTextReceiveTime = null; // 第一个文本消息接收时间
let firstAudioReceiveTime = null; // 第一个音频包接收时间
...
let streamingTextBuffer = '';        // 流式文本缓冲区
...
const timingButton = document.getElementById('timingButton');
            ...
            if (!streamingContext) {
                streamingContext = {
                    ...
                    startPlaying: function () {
                        ...
                        // 记录第一个音频包开始播放时间
                        if (firstAudioPlayTime === null) {
                            firstAudioPlayTime = performance.now();
                            log(`🔊 第一个音频包开始播放时间: ${firstAudioPlayTime.toFixed(2)}ms`, 'info');

                            // 计算所有耗时
                            if (isTiming && timingStartTime !== null) {
                                const textLatency = firstTextReceiveTime ? (firstTextReceiveTime - timingStartTime).toFixed(2) : '未收到';
                                const audioReceiveLatency = firstAudioReceiveTime ? (firstAudioReceiveTime - timingStartTime).toFixed(2) : '未收到';
                                const audioPlayLatency = (firstAudioPlayTime - timingStartTime).toFixed(2);

                                // 构建详细的时延信息
                                const timingInfo = `📊 时延统计:
                                    📝 文本接收: ${textLatency}ms
                                    🎵 音频接收: ${audioReceiveLatency}ms  
                                    🔊 音频播放: ${audioPlayLatency}ms`;

                                // 在按钮附近显示结果
                                timingResult.innerHTML = timingInfo.replace(/\n/g, '<br>');
                                timingResult.style.display = 'flex';

                                // 同时在日志中记录
                                log(`🎯 时延统计完成:`, 'success');
                                log(`📝 开始计时到收到文本: ${textLatency}ms`, 'success');
                                log(`🎵 开始计时到收到音频包: ${audioReceiveLatency}ms`, 'success');
                                log(`🔊 开始计时到播放音频: ${audioPlayLatency}ms`, 'success');

                                // 停止计时状态
                                isTiming = false;
                                timingStartTime = null;
                                firstAudioPlayTime = null;
                                firstTextReceiveTime = null;
                                firstAudioReceiveTime = null;

                                // 更新按钮状态
                                timingButton.textContent = '开始计时';
                                timingButton.classList.remove('timing');
...
/** 麦克风需安全上下文：HTTPS，或 http://localhost / 127.0.0.1；http://局域网IP 下 mediaDevices 常为 undefined。 */
function getMediaDevicesForMic() {
    if (navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === 'function') {
        return navigator.mediaDevices;
    }
    const secure = window.isSecureContext === true;
    const hint = !secure
        ? '当前为 http + 非本机地址（如通过局域网 IP 访问），浏览器不会开放麦克风 API。请为本机 nginx 配置 HTTPS 后用 https://访问，或仅在当前电脑上用 http://localhost 或 127.0.0.1 打开页面再试语音。'
        : '浏览器未提供 mediaDevices.getUserMedia，请使用较新版本浏览器并检查站点麦克风权限。';
    throw new Error('无法使用麦克风（mediaDevices 不可用）。' + hint);
}

// 初始化音频录制和处理
async function initAudio() {
    try {
        const md = getMediaDevicesForMic();
        // 请求麦克风权限
        ...
        websocket.onmessage = function (event) {
            try {
                ...
                if (typeof event.data === 'string') {
                    ...
                    // 处理 hello 响应 - 匹配 handler.py 的格式
                    if (message.type === 'hello') {
                        log(`服务器回应：${JSON.stringify(message, null, 2)}`, 'success');
                    }
                    // 处理 assistant 消息 - 匹配 handler.py 的格式: {"role": "assistant", "content": "..."}
                    // 支持流式文本输出：多个消息片段会累积显示在一个消息中
                    else if (message.role === 'assistant') {
                        const content = message.content || '';

                        // 处理 SENTENCE_START - 开始新的流式响应
                        if (content === 'SENTENCE_START') {
                            log('助手开始回复（流式）', 'info');
                            // 重置流式状态
                            currentStreamingMessage = null;
                            streamingTextBuffer = '';
                            return; // 不显示 SENTENCE_START
                        }

                        // 处理 SENTENCE_END - 结束流式响应
                        if (content === 'SENTENCE_END') {
                            log('助手回复结束（流式）', 'info');
                            // 完成流式消息，清理状态
                            currentStreamingMessage = null;
                            streamingTextBuffer = '';
                            return; // 不显示 SENTENCE_END
                        }

                        // 处理实际的文本内容
                        if (content) {
                            // 记录第一个文本消息接收时间
                            if (isTiming && firstTextReceiveTime === null) {
                                firstTextReceiveTime = performance.now();
                                const textLatency = firstTextReceiveTime - timingStartTime;
                                log(`📝 收到第一个文本片段，耗时: ${textLatency.toFixed(2)}ms`, 'success');
                            }

                            // 累积文本到缓冲区
                            streamingTextBuffer += content;

                            // 如果是第一个片段，创建新的消息元素
                            if (!currentStreamingMessage) {
                                currentStreamingMessage = addMessage(streamingTextBuffer);
                                log(`助手回复（流式开始）: ${content}`, 'info');
                            } else {
                                // 后续片段追加到现有消息
                                updateMessage(currentStreamingMessage, streamingTextBuffer);
                                log(`助手回复（流式更新）: ${content}`, 'debug');
                            }
                        }
                    }
                    // 处理 updates 消息 - 匹配 handler.py 的格式: {"role": "updates", "updates": "..."}
                    else if (message.role === 'updates') {
                        const updates = message.updates || message.content || '';
                        if (updates) {
                            log(`状态更新: ${updates}`, 'info');
                        }
                    }
                    // 兼容旧格式的消息（向后兼容）
                        ...
                        if (isTiming && firstTextReceiveTime === null) {
                            firstTextReceiveTime = performance.now();
                            const textLatency = firstTextReceiveTime - timingStartTime;
                            log(`📝 收到第一个文本消息${message.text}，耗时: ${textLatency.toFixed(2)}ms`, 'success');
                        }
                        ...
                        // 未知消息类型
                        log(`未知消息类型: ${message.type || message.role || 'unknown'}`, 'info');
...
function sendTextMessage() {
    ...
    try {
        // 使用 handler.py 期望的格式: {"role": "user", "content": [{"type": "text", "text": "..."}]}
        const userMessage = {
            role: 'user',
            content: [{
                type: 'text',
...
function startTiming() {
    if (isTiming) {
        ...
        // 隐藏结果显示
        timingResult.style.display = 'none';
        ...
        if (websocket && websocket.readyState === WebSocket.OPEN) {
            const timestamp = Date.now(); // Unix时间戳（秒）
            ...
            log(`⏰ 发送时间戳消息: ${JSON.stringify(timestampMessage)}`, 'info');
            websocket.send(JSON.stringify(timestampMessage));
        } else {
            log('⚠️ WebSocket未连接，无法发送时间戳消息', 'warning');
        }

        timingButton.textContent = '停止计时';
        timingButton.classList.add('timing');

        // 隐藏之前的结果
        timingResult.style.display = 'none';

        log(`⏱️ 开始计时: ${timingStartTime.toFixed(2)}ms`, 'info');
        log(`📊 等待接收: 文本消息、音频包、音频播放`, 'info');
    }
}
...
function initEventListeners() {
    ...
    if (savedWsUrl) {
        serverUrlEl.value = savedWsUrl;
    } else if (serverUrlEl && location.hostname && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
        const wsScheme = location.protocol === 'https:' ? 'wss:' : 'ws:';
        serverUrlEl.value = wsScheme + '//' + location.host + '/ws';
    }
...
function promptsYamlHighlightValue(raw) {
    if (raw === '') return '';
    const v = raw.trim();
    if (v.startsWith('"') && v.endsWith('"') && v.length >= 2) {
        return '<span class="yaml-string">' + promptsYamlEscapeHtml(raw) + '</span>';
    }
    if (v.startsWith("'") && v.endsWith("'") && v.length >= 2) {
        return '<span class="yaml-string">' + promptsYamlEscapeHtml(raw) + '</span>';
    }
    if (/^(true|false|null|~)$/i.test(v)) {
        return '<span class="yaml-bool">' + promptsYamlEscapeHtml(raw) + '</span>';
    }
    if (/^-?\d+(\.\d+)?$/.test(v)) {
        return '<span class="yaml-number">' + promptsYamlEscapeHtml(raw) + '</span>';
    }
    return '<span class="yaml-scalar">' + promptsYamlEscapeHtml(raw) + '</span>';
}

function promptsYamlHighlightLine(line) {
    if (/^\s*#/.test(line)) {
        return '<span class="yaml-comment">' + promptsYamlEscapeHtml(line) + '</span>';
    }
    const listM = line.match(/^(\s*)(-\s+)(.*)$/);
    if (listM) {
        const indentDash = listM[1];
        const dashPart = listM[2];
        const rest = listM[3];
        const prefix = promptsYamlEscapeHtml(indentDash) +
            '<span class="yaml-list-marker">' + promptsYamlEscapeHtml(dashPart) + '</span>';
        const kv = rest.match(/^([a-zA-Z0-9_.-]+)(\s*:\s*)(.*)$/);
        if (kv) {
            return prefix +
                '<span class="yaml-key">' + promptsYamlEscapeHtml(kv[1]) + '</span>' +
                '<span class="yaml-sep">' + promptsYamlEscapeHtml(kv[2]) + '</span>' +
                promptsYamlHighlightValue(kv[3]);
        }
        return prefix + promptsYamlHighlightValue(rest);
    }
    const kv = line.match(/^(\s*)([a-zA-Z0-9_.-]+)(\s*:\s*)(.*)$/);
    if (kv) {
        return promptsYamlEscapeHtml(kv[1]) +
            '<span class="yaml-key">' + promptsYamlEscapeHtml(kv[2]) + '</span>' +
            '<span class="yaml-sep">' + promptsYamlEscapeHtml(kv[3]) + '</span>' +
            promptsYamlHighlightValue(kv[4]);
    }
    return '<span class="yaml-plain">' + promptsYamlEscapeHtml(line) + '</span>';
}

function promptsYamlHighlightDocument(text) {
    const lines = String(text).split('\n');
    return lines.map(promptsYamlHighlightLine).join('\n');
}
...
function promptsYamlGetApiBase() {
    const el = document.getElementById('promptsYamlApiBase');
    return (el && el.value || '').trim().replace(/\/$/, '');
}

function promptsYamlSetStatus(msg, kind) {
    const el = document.getElementById('promptsYamlStatus');
    if (!el) return;
    el.textContent = msg || '';
    el.className = kind || '';
}

function promptsYamlUpdateSaveButtonState() {
...
function promptsYamlIsDirty() {
    const ta = document.getElementById('promptsYamlEditor');
    if (!ta || !promptsYamlState.currentPath) return false;
    return ta.value !== promptsYamlState.baseline;
}

function promptsYamlConfirmLoseChanges() {
    if (!promptsYamlIsDirty()) return true;
    return confirm('当前文件有未保存修改，确定要离开吗？');
}

function promptsYamlBuildTreeNodes(children, ul) {
    (children || []).forEach(function (node) {
        const li = document.createElement('li');
        li.className = 'md-tree-node' + (node.type === 'dir' ? ' md-tree-dir collapsed' : ' md-tree-file');
        const row = document.createElement('div');
        row.className = 'md-tree-row';
        const caret = document.createElement('span');
        caret.className = 'md-tree-caret';
        ...
        if (node.type === 'dir') {
            ...
            sub.className = 'md-tree-list md-tree-children';
            promptsYamlBuildTreeNodes(node.children || [], sub);
            li.appendChild(sub);
...
function promptsYamlClearTreeSelection() {
    document.querySelectorAll('.md-tree-row.active').forEach(function (r) {
        r.classList.remove('active');
    });
}

async function promptsYamlLoadTree() {
    const base = promptsYamlGetApiBase();
    promptsYamlState.apiBase = base;
    const treeEl = document.getElementById('promptsYamlTree');
    if (!treeEl) return;
    treeEl.innerHTML = '';
    if (!base) {
...
async function promptsYamlOpenFile(path, rowEl) {
    ...
    try {
        ...
        promptsYamlState.currentPath = path;
...
async function promptsYamlSave() {
    ...
    try {
        ...
        if (res.status === 204 || res.ok) {
            promptsYamlState.baseline = ta.value;
            promptsYamlSetStatus('已保存', 'ok');
            promptsYamlUpdateSaveButtonState();
            return;
        }
        const data = await res.json().catch(function () { return null; });
        ...
        promptsYamlSetStatus('保存失败: ' + e.message, 'error');
    }
}
...
function initPromptsYamlPanel() {
    ...
    if (apiInput) {
        const savedBase = localStorage.getItem('promptsYamlApiBase');
        if (savedBase) {
            ...
            try {
                const wsInput = document.getElementById('serverUrl');
                const wsUrl = (wsInput && wsInput.value || '').trim();
                if (wsUrl) {
                    const u = new URL(wsUrl);
                    u.protocol = (u.protocol === 'wss:') ? 'https:' : 'http:';
                    u.port = '8010';
                    apiInput.value = u.origin;
                }
            } catch (e) {
                // 忽略推导错误，保留默认值
            }
        ...
        apiInput.addEventListener('change', function () {
            ...
            localStorage.setItem('promptsYamlApiBase', v);
        });
    }
...
async function startDirectRecording() {
    ...
    try {
        ...
        // 注意：handler.py 不期望 listen 消息，音频数据直接发送即可
        // 如果需要通知服务器开始录音，可以发送一个空的用户消息
        // 但通常直接发送音频数据即可
        if (!websocket || websocket.readyState !== WebSocket.OPEN) {
...
function stopDirectRecording() {
    ...
    try {
        ...
        if (websocket && websocket.readyState === WebSocket.OPEN) {
            // 使用空的Uint8Array发送最后一个空帧，表示音频流结束
            const emptyOpusFrame = new Uint8Array(0);
            websocket.send(emptyOpusFrame);
            log('已发送录音停止信号（空音频帧）', 'info');
        }
...
async function handleBinaryMessage(data) {
    try {
        ...
        if (opusData.length > 0) {
            // 记录第一个音频包接收时间
            if (isTiming && firstAudioReceiveTime === null) {
                firstAudioReceiveTime = performance.now();
                const audioReceiveLatency = firstAudioReceiveTime - timingStartTime;
                log(`🎵 收到第一个音频包，耗时: ${audioReceiveLatency.toFixed(2)}ms`, 'success');
            }
